...
void loop()
{
  ...
  if (comando == "ocioso") { novo_estado("OCIOSO"); }
  if (comando == "vigia") { novo_estado("VIGIA"); }
  ...
  if (estado == "OCIOSO") {
    ...
    if (contador < contador_max) {
      ...
      analogWrite(Aux_Verde, estados[1] * 120);
    }
  }
}
...
void novo_estado(String estado_novo) {
  ...
  digitalWrite(LED_alarme, estados[0]);
  digitalWrite(LED_ocioso, estados[1]);
  digitalWrite(Aux_Verde, estados[1]);
  digitalWrite(Som, estados[2]);
}